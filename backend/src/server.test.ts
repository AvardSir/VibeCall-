...
function makeAppAndIo(): { app: ReturnType<typeof createApp>; io: ChatServer } {
  ...
  const io = createSocketServer({ config: { corsOrigin: config.corsOrigin }, admin, chat: chat as never, registry, getGraceRemaining: () => null, startGrace: vi.fn(), cancelGrace: vi.fn() });
