...
- **Dependency on M4:** M6 **retrofits `backend/src/webhooks.ts`** (created by M4) so that when the host enters grace OR the active sharer leaves, the share is force-cleared and `share_state { null }` is broadcast (M4 explicitly deferred this — `2026-07-01-m4-host-lifecycle.md`: "Screen-share force-clear on grace … no-op until M6"). **If M4 is not yet merged**, Task 4 instead adds a minimal `participant_left`→`clearShare` handler wherever participant-left is processed (or notes it as pending until M4 lands). Recommended order: M4 → M6.
- When rebasing onto M4/M5, **MERGE** the socket event maps, `ChatGatewayDeps`/`AppDeps`, `rooms.ts` fields, and the `call` i18n namespace — never replace. `rooms.ts` gains `activeSharerId` (M6) alongside `status`/`graceEndsAt` (M4) and `memberTokens` (M5).
...
- Create: `features/call/hooks/useShareState.ts` (+ test) — subscribe to `share_state`/`share_granted`/`share_denied` → store + a grant/deny promise bridge.
- Create: `features/call/hooks/useScreenShare.ts` (+ test) — `{ isSharing, isBusy, error, toggle }`: claim→picker→publish / stop→release, LocalTrackUnpublished detection, 4s error auto-dismiss.
- Create: `features/call/components/ScreenShareView.tsx` (+ test) — contain-fit share track + label.
- Create: `features/call/components/ThumbnailStrip.tsx` (+ test) — horizontal `VideoTile` row (reuses the M2 tile).
- Modify: `features/call/components/VideoGrid.tsx` — extract the per-participant tile mapping so grid + strip share it (or accept a `layout` prop).
...
Update the deterministic-generator `toEqual` (M3 Task 1) to include `activeSharerId: null` (or switch it to `toMatchObject`).
...
- [ ] **Step 1: Implement** — change the socket-server construction to include the registry:

```ts
  createSocketServer(httpServer, { config, admin, chat, registry });
```

(The `registry` is already constructed in `server.ts`; this just threads it into the gateway deps so the share handlers can arbitrate.)

- [ ] **Step 2: Backend typecheck** — `cd backend && npm run typecheck` → clean (confirms `ChatGatewayDeps` now requires `registry` and it's supplied).
...
- [ ] **Step 3: Implement** — add `setActiveSharerId: (id) => set({ activeSharerId: id })` to the store and its state type; add the five events to `socketEvents.ts` (mirror backend, keep the cross-ref comment).
...
### Task 7: `useShareState` hook (socket → store + grant bridge)

**Files:** Create `frontend/src/features/call/hooks/useShareState.ts`, `useShareState.test.ts`.

**Interfaces:** `useShareState(): { requestShare: () => Promise<'granted' | 'busy'> }`. It subscribes (via `useSocket()`) to `share_state` → `setActiveSharerId`, and bridges `claim_share`→(`share_granted`|`share_denied`) into a promise: `requestShare()` emits `claim_share { roomName }` (roomName from `useConnectionStore.localParticipant.roomId`) and resolves `'granted'` on the next `share_granted` or `'busy'` on the next `share_denied` (with a timeout fallback → `'busy'`). Off the exact handlers on cleanup.

- [ ] **Step 1: Test (red)** — mock `useSocket` capturing handlers; assert `share_state` → store; `requestShare()` emits `claim_share` and resolves `'granted'` when the captured `share_granted` handler is invoked, `'busy'` when `share_denied` is.
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement**:
...
export function useShareState(): { requestShare: () => Promise<'granted' | 'busy'> } {
  const socket = useSocket();
  const roomId = useConnectionStore((s) => s.localParticipant?.roomId ?? '');
...
- [ ] **Step 4: Run** → PASS. **Step 5: Commit** — `git commit -m "feat(frontend): useShareState — share_state subscription + claim/grant bridge"`.
...
**Interfaces:** `useScreenShare(): { isSharing: boolean; isBusy: boolean; error: string | null; toggle: () => void }`. Uses `useLocalParticipant()`, `useShareState()` (Task 7), `useParticipantsStore(activeSharerId)`. `isSharing = activeSharerId === localIdentity`; `isBusy = activeSharerId !== null && activeSharerId !== localIdentity`. `toggle()`: if sharing → `setScreenShareEnabled(false)` + `socket release_share`; else if not busy → `await requestShare()`; on `'granted'` → `try setScreenShareEnabled(true) catch → release_share + set error (call.shareError)`; on `'busy'` → set error (call.shareBusy). Errors auto-dismiss after 4s. Also listens for `RoomEvent.LocalTrackUnpublished` (source ScreenShare) while sharing → emit `release_share`.

- [ ] **Step 1: Test (red)** — mock `useLocalParticipant` (`setScreenShareEnabled` spy), `useShareState` (`requestShare` returning `'granted'`/`'busy'`), the socket (`emit` spy), and the store. Cases: idle + grant → `setScreenShareEnabled(true)` called; grant but `setScreenShareEnabled` rejects → `release_share` emitted + `error === shareError`; busy → `error === shareBusy`, picker not opened; sharing + toggle → `setScreenShareEnabled(false)` + `release_share`.
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** the hook per the interface. Use `useTranslation('call')` for the error strings; a `useRef` timer for the 4s auto-dismiss (cleared on unmount / on the next error). Guard `setScreenShareEnabled(true)` in try/catch; on catch emit `release_share` and set `t('shareError')`.
...
**Interfaces:** Extract the participant→`VideoTile` mapping from `VideoGrid` into a small helper/subcomponent so both the grid and the strip render identical tiles (same name/camera-off/mute logic, host-first order). `ThumbnailStrip` renders that mapping in a horizontal, fixed-height, scrollable row (`flex gap-2 overflow-x-auto`), each tile at thumbnail size with `object-cover`.

- [ ] **Step 1: Test (red)** — seed `useParticipantsStore.participants` with two participants + mock `useTracks([Track.Source.Camera])`; render `ThumbnailStrip`; assert both names render and the container is a horizontal flex row (query by a testid or role). Assert camera-off participant shows the mic-state icon (reuse VideoTile's existing behavior — the test mirrors the VideoGrid tile test).
- [ ] **Step 2: Run** → FAIL.
- [ ] **Step 3: Implement** — refactor `VideoGrid` to call a shared `renderParticipantTiles(participants, cameraTracks, localIdentity)` (or a `<ParticipantTiles/>` inner component) used by both; `ThumbnailStrip` wraps it in the horizontal row. Keep `VideoGrid`'s grid layout + `Waiting…` behavior unchanged. Do not alter `VideoTile`.
...
**Interfaces:** `CallShell` reads `activeSharerId` from `useParticipantsStore` and calls `useShareState()` (mounts the `share_state` subscription). When `activeSharerId !== null`, the main content renders `<ScreenShareView />` (flex-1) above `<ThumbnailStrip />` (fixed height); otherwise `<VideoGrid />` as today. The `Waiting for someone to join…` empty state remains visible over the share layout when the participant count is 1 (reuse the existing overlay, or render it in `ScreenShareView`'s placeholder). `useScreenShare` is consumed inside `ControlsBar` (Task 11), so `CallShell` only needs `useShareState` for the subscription + the layout branch.
...
**Type consistency:** the five socket events are identical FE (`socketEvents.ts`, Task 6) ↔ BE (`socket.ts`, Task 2). `activeSharerId: string | null` consistent across registry (1), `share_state` payload (2/6), store (6), and the layout switch (12). `requestShare(): Promise<'granted'|'busy'>` (7) consumed by `useScreenShare` (8) consumed by `ControlsBar` (11).
